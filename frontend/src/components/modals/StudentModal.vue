    ...
    <div class="bg-white rounded-xl shadow-xl max-w-md w-full overflow-hidden border border-gray-100">
      ...
      <form @submit.prevent="handleSubmit" class="p-6 space-y-4">
        <div>
          ...
          <input v-model="formData.studentCode" type="text" class="w-full px-3.5 py-2 border border-gray-300 rounded-lg text-sm disabled:bg-gray-100" required />
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">Họ và tên</label>
          <input v-model="formData.fullName" type="text" class="w-full px-3.5 py-2 border border-gray-300 rounded-lg text-sm" required />
        </div>
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Ngày sinh</label>
            <input v-model="formData.dob" type="date" class="w-full px-3.5 py-2 border border-gray-300 rounded-lg text-sm" required />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Giới tính</label>
            <select v-model="formData.gender" class="w-full px-3.5 py-2 border border-gray-300 rounded-lg text-sm">
        ...
        <div>
          ...
          <select v-model="formData.classId" :disabled="disableClassSelection" class="w-full px-3.5 py-2 border border-gray-300 rounded-lg text-sm disabled:bg-gray-100" required>
            <option disabled value="">Chọn lớp...</option>
            <option v-for="c in classes" :key="c.id" :value="c.id">{{ c.code }} - {{ c.name }}</option>
          </select>
        </div>
        <div class="flex justify-end gap-3 pt-2">
          <button type="button" @click="$emit('close')" class="px-4 py-2 text-sm text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50">Hủy</button>
          <button type="submit" class="px-4 py-2 text-sm text-white bg-blue-600 rounded-lg hover:bg-blue-700">{{ editData ? 'Cập nhật' : 'Thêm mới' }}</button>
        </div>
      </form>
    </div>
  </div>
